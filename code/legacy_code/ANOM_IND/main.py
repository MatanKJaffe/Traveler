    ...
    with fiona.Env(CPL_DEBUG=True):
        ...
        with rio.Env(CPL_DEBUG=True, GDAL_CACHEMAX=1073741824):
            with fiona.open(roof_db_shapefile, "r") as shapefile:
                ...
                split_shapes = np.array_split(shapes, 17)
                def run_calc(shapes):
                    with rio.open(in_tiff,"r",format='GTiff',dtype = np.uint8) as src:
                        bright_lst = [gpd.GeoDataFrame(gpd.GeoDataFrame({'geometry':[]})) for _ in range(len(Radius_px_lst))]
                        dark_lst  = [gpd.GeoDataFrame(gpd.GeoDataFrame({'geometry':[]})) for _ in range(len(Radius_px_lst))]
                        for shape in tqdm(shapes):
                            ...
                            for idx, radius_lst in enumerate(Radius_px_lst):
                                ...
                                #dark_lst[idx].set_crs(src.crs)
                                #dark_dict[idx].to_file(driver = 'ESRI Shapefile', filename=out_shapefile+f"dark_{radius_lst}.shp")
                        return [bright_lst,dark_lst]
            
                #1result_list_tqdm = progress_map(func = run_calc, tasks = shapes, n_cpu=multiprocessing.cpu_count()+1, chunk_size= 16, core_progress=True, context='spawn', total=None, bar_step=1,
                #disable=False)
                with Pool(multiprocessing.cpu_count()+1) as pool:
                    result_list_tqdm = []
                    #for position in range(multiprocessing.cpu_count()+1):
                    for result in tqdm(pool.imap(func=run_calc, iterable=split_shapes), total=len(split_shapes)):
                ...
                with rio.open(in_tiff,"r",format='GTiff',dtype = np.uint8) as src:
                    for idx in tqdm(range(len(Radius_px_lst))):
                        bright_gdf = gpd.GeoDataFrame(gpd.GeoDataFrame({'geometry':[]}),crs = src.crs)
                        dark_gdf = gpd.GeoDataFrame(gpd.GeoDataFrame({'geometry':[]}),crs = src.crs)
